...
| **Linux Docker 核心后端 + Windows `mt5_gateway`** | ✅ | ⭐⭐⭐⭐ | 核心后端配置 `MT5_GATEWAY_URL`；网关在 Windows 本机连 MT5。见 `mt5_gateway/README.md`。 |

### 分栈部署：Linux 核心后端 + Windows MT5 网关

在 Linux（或 WSL）用 Docker 跑 Postgres/Redis/后端；在装有 MT5 Terminal 的 Windows 上：

1. 启动网关：`py -m mt5_gateway.run`，并设置 `MT5_GATEWAY_API_KEY`。
2. 在 `backend_api_python/.env` 中：`MT5_GATEWAY_URL=http://<windows-ip>:5100`，`MT5_GATEWAY_API_KEY` 与网关一致，`ALLOW_LOCAL_DESKTOP_BROKERS=true`。

Broker 页面（`/api/mt5/*`）与外汇实盘策略均走网关。单网关实例建议 UI 与策略使用同一 MT5 账号。
