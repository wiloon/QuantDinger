...
| **Linux Docker core + Windows `mt5_gateway`** | ✅ | ⭐⭐⭐⭐ | Core backend sets `MT5_GATEWAY_URL`; gateway runs on Windows next to MT5 Terminal. See `mt5_gateway/README.md`. |

### Split stack: Linux core backend + Windows MT5 gateway

Run Postgres/Redis/backend in Docker on Linux (or WSL). On a Windows machine with MT5 Terminal:

1. Start gateway: `py -m mt5_gateway.run` with `MT5_GATEWAY_API_KEY` set.
2. In `backend_api_python/.env`: `MT5_GATEWAY_URL=http://<windows-host>:5100`, same `MT5_GATEWAY_API_KEY`, `ALLOW_LOCAL_DESKTOP_BROKERS=true`.

Broker UI (`/api/mt5/*`) and live forex strategies both use the gateway. Use one MT5 account for UI and strategies when possible.
