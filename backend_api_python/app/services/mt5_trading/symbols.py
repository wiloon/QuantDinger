...
def _normalize_symbol_base(part: str) -> str:
    """Uppercase the instrument root; strip common separators."""
...
def normalize_symbol(symbol: str, broker_suffix: str = "") -> str:
    ...
    - No suffix: "EURUSD"
    - With suffix: "EURUSDm", "EURUSD.raw", "EURUSD.z" (NordFX-style)
    
    Args:
        symbol: Symbol code (e.g., "EUR/USD", "EURUSD", "eurusd", "EURUSD.z")
    ...
    # Preserve broker suffix case after the last dot (e.g. EURUSD.z, not EURUSD.Z).
    if "." in raw:
        base, suffix = raw.rsplit(".", 1)
        base_norm = _normalize_symbol_base(base)
        suffix_clean = suffix.strip()
        if base_norm and suffix_clean:
            return f"{base_norm}.{suffix_clean}"
        if base_norm:
            return base_norm

    normalized = _normalize_symbol_base(raw)

    if broker_suffix:
        suf = broker_suffix.strip()
        if suf and not normalized.endswith(suf.upper()) and not normalized.endswith(suf):
            normalized = normalized + suf
